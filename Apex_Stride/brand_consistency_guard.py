        ...
        if conversation_type == "complaint_resolution":
            ...
            if not any(w in response_lower for w in r["required_acknowledgment_words"]):
                violations.append(BrandViolation(severity="critical", category="complaint_tone",
                    ...
                    suggestion="Acknowledge with care: 'I'm really sorry this happened.' 'That's completely understandable.'"))
            if not any(w in response_lower for w in r["required_solution_words"]):
                violations.append(BrandViolation(severity="critical", category="complaint_tone",
                    ...
                    suggestion="Reassure and explain: 'Here's exactly how we'll make this right.' 'We'll get this resolved for you.'"))
            if not any(w in response_lower for w in ["sorry", "apologize", "apologies"]):
                violations.append(BrandViolation(severity="medium", category="complaint_tone",
                    description="Missing apology in complaint response",
                    suggestion="Open with empathy: 'I'm sorry you experienced this.' 'We sincerely apologize.'"))
